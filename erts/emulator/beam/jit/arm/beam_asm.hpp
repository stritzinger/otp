...
protected:
    ...
    const a32::Gp scheduler_registers = a32::x19;

    const a32::Gp E = a32::x20;
    const a32::Gp c_p = a32::x21;
    const a32::Gp FCALLS = a32::w22;
    const a32::Gp HTOP = a32::x23;
    ...
    const a32::Gp active_code_ix = a32::x24;
...
#define ERTS_LOWEST_CALLER_SAVE_XREG (ERTS_HIGHEST_CALLEE_SAVE_XREG + 1)
    ...
    void emit_are_both_immediate(a32::Gp Reg1, a32::Gp Reg2) {
    ...
    void sub(a32::Gp to, a32::Gp src, int32_t val) {
        if (val < 0) {
            ...
            a32::Gp super_tmp = follow_size(SUPER_TMP, to);
    ...
    void cmp(a32::Gp src, int32_t val) {
        if (Support::isUInt12(val)) {
            a.cmp(src, imm(val));
        } else if (Support::isUInt12(-val)) {
            a.cmn(src, imm(-val));
        } else {
            a32::Gp super_tmp = follow_size(SUPER_TMP, src);
    ...
    void lea(a32::Gp to, arm::Mem mem) {
        ...
        if (offset == 0) {
            a.mov(to, a32::GpX(mem.baseId()));
        } else {
            add(to, a32::GpX(mem.baseId()), offset);
        }
    }
};
...
class BeamModuleAssembler : public BeamAssembler,
    ...
    void mov_imm_preserve_cache(a32::Gp dst, UWord value) {
        if (a.offset() == last_destination_offset) {
            mov_imm(dst, value);
            preserve__cache(dst);
        } else {
...
protected:
    ...
    void emit_extract_binary(const a32::Gp bitdata,
                             Uint bits,
                             const ArgRegister &Dst);
    ...
    bool emit_optimized_three_way_select(a32::Gp reg,
                                         Label fail,
                                         const Span<ArgVal> &args);
...
#include "beamasm_protos.h"
    ...
    void safe_stp(a32::Gp gp1, a32::Gp gp2, arm::Mem mem) {
        ...
        if (abs_offset <= sizeof(Eterm) * MAX_LDP_STP_DISPLACEMENT) {
            ...
            add(SUPER_TMP, a32::GpX(mem.baseId()), offset);
