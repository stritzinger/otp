...
typedef std::pair<Uint32, Uint32> op_stats;
...
void BeamGlobalAssembler::emit_i_breakpoint_trampoline_shared() {
    ...
    a.ldrb(ARG1.w(), arm::Mem(a32::x30, -flag_offset));
...
void BeamGlobalAssembler::emit_i_func_info_shared() {
    /* a32::x30 now points 4 bytes into the ErtsCodeInfo struct for the
     * function. Put the address of the MFA into ARG1. */
    a.add(ARG1, a32::x30, offsetof(ErtsCodeInfo, mfa) - 4);
...
void BeamModuleAssembler::emit_constant(const Constant &constant) {
    ...
    if (value.isImmed()) {
        ...
        case ArgVal::Catch: {
             ...
             * this as a 32-bit value with the upper bits cleared. */
