...
protected:
    ...
    const arm::Gp scheduler_registers = a32::x19;

    const arm::Gp E = a32::x20;
    const arm::Gp c_p = a32::x21;
    const arm::Gp FCALLS = a32::w22;
    const arm::Gp HTOP = a32::x23;
    ...
    const arm::Gp active_code_ix = a32::x24;
...
#    define ERTS_HIGHEST_CALLER_SAVE_XREG 5
    const arm::Gp XREG0 = a32::x25;
    const arm::Gp XREG1 = a32::x26;
    const arm::Gp XREG2 = a32::x27;

    /*
     * Caller-save X registers. Must be flushed before calling C
     * code.
     */
    const arm::Gp XREG3 = a32::x15;
    const arm::Gp XREG4 = a32::x16;
    const arm::Gp XREG5 = a32::x17;
#else
...
#    define ERTS_HIGHEST_CALLER_SAVE_XREG 5
    ...
    const arm::Gp XREG1 = a32::x26;
    const arm::Gp XREG2 = a32::x27;
    const arm::Gp XREG3 = a32::x28;

    /*
     * Caller-save X registers. Must be flushed before calling C
     * code.
     */
    const arm::Gp XREG4 = a32::x15;
    const arm::Gp XREG5 = a32::x16;
#endif
...
#define ERTS_LOWEST_CALLER_SAVE_XREG (ERTS_HIGHEST_CALLEE_SAVE_XREG + 1)
     ...
     */
    const arm::Gp ARG1 = a32::x0;
    const arm::Gp ARG2 = a32::x1;
    const arm::Gp ARG3 = a32::x2;
    const arm::Gp ARG4 = a32::x3;
    const arm::Gp ARG5 = a32::x4;
    const arm::Gp ARG6 = a32::x5;
    const arm::Gp ARG7 = a32::x6;
    const arm::Gp ARG8 = a32::x7;

    const arm::Gp TMP1 = a32::x8;
    const arm::Gp TMP2 = a32::x9;
    const arm::Gp TMP3 = a32::x10;
    const arm::Gp TMP4 = a32::x11;
    const arm::Gp TMP5 = a32::x12;
    const arm::Gp TMP6 = a32::x13;

    /*
     * Assume that SUPER_TMP will be destroyed by any helper function.
     */
    const arm::Gp SUPER_TMP = a32::x14;
     ...
     *
     * Note that only the bottom 32 bits of these (128-bit) registers are
     * callee-save, so we cannot pack two floats into each register. */
    const arm::VecD FREG0 = a32::d8;
    const arm::VecD FREG1 = a32::d9;
    const arm::VecD FREG2 = a32::d10;
    const arm::VecD FREG3 = a32::d11;
    const arm::VecD FREG4 = a32::d12;
    const arm::VecD FREG5 = a32::d13;
    const arm::VecD FREG6 = a32::d14;
    const arm::VecD FREG7 = a32::d15;
    ...
    static const Uint32 bad_boxed_ptr = 0xcafebad0000002UL;
    ...
    void emit_enter_erlang_frame() {
        a.str(a32::x30, arm::Mem(E, -8).pre());
    }

    void emit_leave_erlang_frame() {
        a.ldr(a32::x30, arm::Mem(E).post(8));
    }
    ...
    void emit_leave_runtime_frame() {
        a.mov(a32::sp, a32::x29);
        a.ldp(a32::x29, a32::x30, arm::Mem(a32::sp).post(16));
    }
    ...
    void sub(arm::Gp to, arm::Gp src, int32_t val) {
        if (val < 0) {
            add(to, src, -val);
    ...
    void add(arm::Gp to, arm::Gp src, int32_t val) {
        if (val < 0) {
            sub(to, src, -val);
        } else if (val == 0 && to != src) {
            a.mov(to, src);
    ...
    void subs(arm::Gp to, arm::Gp src, int32_t val) {
    ...
    void cmp(arm::Gp src, int32_t val) {
        if (Support::isUInt12(val)) {
            a.cmp(src, imm(val));
        } else if (Support::isUInt12(-val)) {
            a.cmn(src, imm(-val));
    ...
    void ldur(arm::Gp reg, arm::Mem mem) {
        safe_9bit_imm(a32::Inst::kIdLdur, reg, mem);
    }

    void stur(arm::Gp reg, arm::Mem mem) {
        safe_9bit_imm(a32::Inst::kIdStur, reg, mem);
    }
