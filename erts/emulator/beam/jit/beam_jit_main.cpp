...
// #if defined(__aarch32__) && !(defined(WIN32) || defined(__APPLE__)) &&         \
...
extern "C"
{
    ...
    void beamasm_flush_icache(const void *address, size_t size) {
        ...
        __asm__ __volatile__("dsb ish\n" ::: "memory");
#elif defined(__aarch32__) && defined(BEAMASM_MANUAL_ICACHE_FLUSHING)
        ...
        stride = min_dcache_line_size;
        for (UWord i = start & ~(stride - 1); i < end; i += stride) {
            __asm__ __volatile__("dccmvau, %0\n" ::"r"(i) :);
        }
        ...
        stride = min_icache_line_size;
        for (UWord i = start & ~(stride - 1); i < end; i += stride) {
            __asm__ __volatile__("icimvau, %0\n" ::"r"(i) :);
        }
