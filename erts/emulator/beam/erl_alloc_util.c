...
extern void erts_alloc_trace_carrier_create(const char *alloc_name,
                                            int alloc_ix,
                                            UWord carrier_size,
                                            int is_mseg,
                                            int is_sbc,
...
cpool_insert(Allctr_t *allctr, Carrier_t *crr)
{
    ...
    /*
     * We begin by passing one element before trying to insert. This in order to
...
/*
 * Link 'link' into list by searching in 'next' direction and insert somewhere
 * after 'start_after' but not further away than after 'end_after'.
 * Node 'end_after' must be guaranteed to not be deleted (sentinel or busy).
 */
static void
...
cpool_unlink(ErtsAlcCPoolData_t *unlink)
{
    ...
    /* Not allowed to free or re-insert this one until thread progress */
...
create_carrier(Allctr_t *allctr, Uint umem_sz, UWord flags)
{
 ...
 try_sys_alloc:
    ...
    if (
        ...
        Uint mseg_no = allctr->mbcs.carriers[ERTS_CRR_ALLOC_MSEG].no
                       + allctr->sbcs.carriers[ERTS_CRR_ALLOC_MSEG].no;
        char main_name[160];
        if (mseg_no == 1) {
            erts_snprintf(main_name, sizeof(main_name),
                          "initial_%s_%u_carrier_%u",
                          ERTS_ALC_A2AD(allctr->alloc_no),
                          (unsigned int) allctr->ix,
                          (unsigned int) mseg_no);
        }
        else {
            erts_snprintf(main_name, sizeof(main_name),
                          "%s_%u_carrier_%u",
        ...
        main_name[sizeof(main_name) - 1] = '\0';
        (void) erts_mmap_name_mapping_global(crr, crr_sz, main_name);
    }

    erts_alloc_trace_carrier_create(ERTS_ALC_A2AD(allctr->alloc_no),
                                    allctr->ix,
                                    crr_sz,
#if HAVE_ERTS_MSEG
                                    IS_MSEG_CARRIER(crr) ? 1 : 0,
#else
                                    0,
#endif
                                    (flags & CFLG_SBC) ? 1 : 0,
